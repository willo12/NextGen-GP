...
char* instrset = "ASMIG"; /* used at runtime */
char* instrset_rare = "T"; /* used at runtime */
char* instrsetswap = "ASMG"; /* instr that can swap */
...
int gridtraining(Node *player, Experiment Exp);
...
int gridtraining(Node *player, Experiment Exp)
{
  ...
  for (o=0;o<25;o++)
  {
    ...
    reg[0].value = location[0];
    reg[1].value = location[1];

    reg[2].value = rand()%(max_board[0]+1);
    reg[3].value = rand()%(max_board[1]+1);
    reg[4].value = last_move;
    ...
    {
      location[0] += 1;
      if (location[0] > max_board[0])
      {
        location[0] = max_board[0];
      }
    }
    else if (move==2)
    {
    ...
    {
      location[1] += 1;
      if (location[1] > max_board[1])
      {
        location[1] = max_board[1];
      }
    } 
  }
...
int gridgame(Node *players[], Experiment Exp)
{
  ...
  for (o=0;o<50;o++)
  {
    for (i=0;i<2;i++)
    {
      reg[0].value = location[2*i];
      reg[1].value = location[2*i+1];

      reg[2].value = location[2*(1-i)];
      reg[3].value = location[2*(1-i)+1];
