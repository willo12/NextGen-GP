...
cdef extern void c_map_f(char treestring[], double* A, double* B, int shape_i, int shape_j, double S0_start, double S1_start,double S0_end, double S1_end, double * forcing, int forcing_dim, int run_mode)
...
def map_f(char * treestring, np.ndarray[double, ndim=2, mode="c"] A not None,np.ndarray[double, ndim=2, mode="c"] B not None ,np.ndarray[double, ndim=1, mode="c"] forcing not None,forcing_dim=1, S0_start=-2., S1_start=-2., S0_end=2., S1_end=2.,run_mode=0):
  """
  ...
  c_map_f(treestring, &A[0,0], &B[0,0], shape_i, shape_j, S0_start, S1_start, S0_end, S1_end, &forcing[0],forcing_dim, run_mode)
